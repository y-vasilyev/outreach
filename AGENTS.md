...
### 14. DiscoveryQueryPlanner — `discovery_query_planner`
Направляемый поиск блогеров (ajtbd-guided-blogger-discovery). Превращает цель/AJTBD кампании или ручной бриф в ограниченный план веб-поиска: несколько разных поисковых запросов с платформой, обоснованием, ожидаемым сигналом аудитории, негативными словами и confidence. `query` — плейн-текст ниши без операторов `site:`/`inurl:` (scope добавляет search core). Воркер `guided-discovery` детерминированно усекает план до `budgets.maxQueries` и пишет trace-событие об усечении. Агент дополнительно вычищает любые просочившиеся операторы и дедуплицирует запросы. Пустой план → запуск помечается `failed`, web-поиск не выполняется. **Модель**: medium (Flash-class).
