...
Support an explicit `bot` contact type for advertising/intake Telegram bots (e.g. a blogger publishing "по рекламе — @hadeout_bot"), so the system can extract, store, surface, and route such contacts correctly — reachable via Telegram DM but excluded from automated human-framed `agency_sourcing` sends so an operator handles them.
