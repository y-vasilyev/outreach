...
Provide an asynchronous, operator-supervised guided blogger discovery workflow that turns a campaign goal/AJTBD (or a manual brief) into a bounded plan of public web-search queries, executes them with a fully traceable run log, enriches candidates with public evidence, and uses LLM agents to plan queries and review candidate fit — all behind the runtime `channel_discovery` flag, with per-run cost/budget controls and an operator workbench UI.
