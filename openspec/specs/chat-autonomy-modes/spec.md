...
- **WHEN** the schema migration runs on a database with conversations or campaigns whose mode is the legacy `auto`
- **THEN** all such rows are updated to `semi_auto` before the new `auto` semantics are introduced
