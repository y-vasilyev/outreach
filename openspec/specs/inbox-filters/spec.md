...
Filters for the inbox conversation list — both the API contract (`GET /conversations` filter params, `q` free-text search, normalisation rules) and the web inbox UX (URL as single source of truth, push/replace history split, deep-link from campaign detail, right-pane fallback when the selected conversation falls outside the active filter). The shared `ConversationFiltersZ` schema normalises empty/whitespace inputs and caps `q` so the downstream `ILIKE` scan stays bounded.
