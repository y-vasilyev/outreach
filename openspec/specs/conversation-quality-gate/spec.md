...
Model-driven goal-fit gate that runs in the `on_inbound` pipeline for `semi_auto` / `auto` conversations. Composes with `SafetyFilter` and `HandoffDecider` to decide auto-send eligibility, applies hysteresis so single bad turns don't flap, and persists each decision so the operator UI can surface the latest verdict.
