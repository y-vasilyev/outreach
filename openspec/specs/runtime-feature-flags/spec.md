...
Operational rollout/kill-switch feature flags live in the DB (`feature_flag`) and are toggled from the admin UI (Settings → Features). A synchronous in-process cache + Redis pub/sub invalidation keep API and worker processes in sync without a redeploy; an env override (`FEATURE_<KEY>_FORCE`) is a hard kill-floor for incidents; fail-safe defaults (off) apply when the store is unreachable.
## Requirements
