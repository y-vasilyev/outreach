...
A `CampaignTypeBuilder` flow that turns a plain-language goal description into a draft campaign type plus drafted agent configs for each pipeline role, runs those agents against test fixtures via the existing `dry_run` path, and — only on explicit operator save — persists editable, versioned `agent_config` rows and the `campaign_type`.
