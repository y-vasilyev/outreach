...
When a new offer row arrives whose identity key matches an `active` row of the same profile (identity key = the existing `offerDedupeKey()` **minus** `price`, `currency` and `rawSnippet` — i.e. platform, kind, duration, tariff_name, slot, **plus `price_period`**; note the current dedupe key does NOT include `delete_policy`, and the identity key follows it — delete_policy stays in `attributes`):
...
Rationale for dropping price from the identity key: with price included (as in roll-up dedupe today) a price change would create a second `active` row instead of a history chain. The roll-up's behavior is preserved anyway because only `active` rows roll up. `rawSnippet` is dropped for the same reason. `price_period` (v2 attribute, `base|seasonal|promo`) joins the identity — a marked seasonal price («Цена июня») is a CONCURRENT offer next to the base price, not a price change; without it the seasonal quote would wrongly supersede the base one (the `placement-representation-v2` «Seasonal price marked, not overwritten» requirement). When extraction didn't mark the period, base/season collapse into one chain (newest active, prior in history) — accepted, nothing is lost. Extending identity with `delete_policy` (arguably «с удалением»/«без удаления» are different products) is deliberately NOT done now — it would diverge from the established dedupe semantics and change backfill replay; it remains a future extension once extraction emits it reliably.
