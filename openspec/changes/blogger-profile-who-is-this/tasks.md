...
- [x] 3.1 `GET /blogger-post-insights/:id/image-url`: 404 insight-missing; 409 unsupported/no-image/storage-off; YouTube → return the public thumbnail URL; Telegram → headObject + presign, else repair via `downloadPublicPostMedia` then presign (502 on refetch failure).
...
- [x] 3.4 Tests: image-url presigns when present; missing Telegram object triggers repair; unsupported → 409; ocrStatus present on a message attachment.

## 4. Web: profile card (apps/web)

- [x] 4.1 Generalize `AttachmentImage` → a reusable `S3Image` (urlFetcher + fallback state); use it for inbound attachments (no behavior change) and post images.
- [x] 4.2 `BloggerProfilePage`: post-example image gallery (S3Image) + per-platform audience table (`platformAudience`) + v2 placement terms (tariff/slot/season/prepayment/tax) on offers.
- [x] 4.3 Web test: post gallery renders images via the fetcher; per-platform audience shows.

## 5. Web: catalog search + compare (apps/web)

- [x] 5.1 `BloggerCatalogPage`: free-text search (name/topic/handle) client filter.
- [x] 5.2 Compare mode: select 2–4 bloggers → a side-by-side panel (per-platform audience, prices per format, formats, freshness).
- [x] 5.3 Web test: search filters; compare renders selected bloggers side by side.

## 6. Web: operator controls (apps/web) — wire the existing backend

- [x] 6.0 Add `message.extraction_status.changed` to web `RealtimeEvents` + subscribe in `ConversationView` (patch the message in cache / invalidate `conversation-messages`); add `ocrStatus` to web `MessageAttachment`/`MediaAsset` types.
- [x] 6.1 Inbox message row: extraction-status badge + «Переанализировать» button (`POST …/reanalyze`); live update on `message.extraction_status.changed`.
- [x] 6.2 Profile data-points table: inline correct (operator data-point write) + delete control.
- [x] 6.3 Extraction-hint form (create a hint, scope channel/conversation/global) from a failed/empty message or the profile.
- [x] 6.4 Attachment chip: show `ocrStatus` («распознано/не удалось»).
