...
- **DB**: Prisma migration `10_guided_discovery_enriching` (additive, mirrors `8_guided_discovery_runs`): new `enriching` value on `discovery_run_status` enum; `@@index([channelId])` on `discovery_run_candidate`; new nullable `review_claimed_at` (DateTime, atomic review claim) and `launched_campaign_id` (String, launch provenance) columns. No destructive changes.
