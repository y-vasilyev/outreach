...
- **Инбокс: фильтр по направлению переписки + «Показать ещё».** В панели
  ...
  и «Мне ответили» (есть входящее). Все контролы фильтров теперь подписаны
  (Кампания / Переписка / Статус / Режим / Поиск), а быстрые чипы над списком
  получили текстовые метки. Список больше не обрывается молча на 100 диалогах —
  внизу кнопка «Показать ещё» подгружает следующую сотню (растущее окно, без
  скачков порядка при живом обновлении).
