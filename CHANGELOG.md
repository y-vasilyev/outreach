...
- **Страница блогера — от аудита к решению.** Профиль блогера перестроен под
  вопрос «подходит ли мне этот блогер»: шапка с именем, ссылками на все
  платформы (основная помечена), темами и форматами — без внутренних
  идентификаторов; полоса решающих метрик (подписчики по платформам,
  ср. просмотры, охват, ERR и ER постов, «цена от ₽», «CPM от ₽») с
  inline-бейджами свежести у каждой цифры; размещения сгруппированы по
  платформам и подняты выше топ-постов. Таблица точек данных, операторские
  правки, extraction hints, полная таблица свежести и внутренние ID собраны
  в свёрнутую секцию «Аудит данных» внизу страницы.

- **Динамика показателей блогера.** Числовые метрики (подписчики по
  платформам, средние просмотры, охват) теперь копят историю изменений:
  на тайлах метрик появились дельты («↑ +12% · 30 дн») и мини-спарклайны,
  а у каждого размещения — изменение к прошлой цене этого же формата
  («↑ +18% к прошлой цене»), посчитанное по истории прайсов. История
  до включения снапшотов достраивается из прежних поколений извлечённых
  фактов.

- **Вердикт соответствия брифу на странице блогера.** Если открыть профиль
  из каталога с активным контекстом кампании/брифа, в шапке появляется
  «Совпадение с брифом: N%» с объяснением, плюсами и пробелами — тот же
  скоринг, что и в каталоге.

- **Переразбор без потери истории.** «Переанализировать» больше не стирает
  прежние извлечённые данные: старое поколение фактов помечается заменённым
  и доступно через API профиля (`?includeSuperseded=true`) — видно, что именно
  изменил переразбор. Каждый извлечённый факт теперь ссылается на конкретный
  запуск агента (модель/конфиг/стоимость) — спорная цена прослеживается до
  источника. Заменённые предложения атрибутов сохраняются как история
  (`?includeSuperseded=true` в ревью атрибутов).
