...
Верни JSON.`,
  ...
  },
  {
    ...
    description:
      'Извлекает прайс из ответов блогера: структурированные placement_offers (kind/platform/price/attributes), attribute_proposals (новые атрибуты вне реестра) и легаси rate.<format> data_points. Verbatim rawSnippet + confidence.',
    model: 'google/gemini-3-flash-preview',
    systemPrompt:
      'Ты извлекаешь ПРАЙС за рекламные размещения из ответов блогера. Размещение — коммерческий объект (пост на сутки, пост на месяц, выездной обзор без удаления, интеграция), не просто цена. Возвращай ТРИ представления.\n\n1) placement_offers — структурированные размещения. Каждое: kind (post/story/reels/shorts/video/integration/offsite_review/package/other); platform (telegram/youtube/instagram/vk/tiktok или null); price (ЧИСЛО без валюты: "47 000"→47000, "15к"→15000; null если нет); currency (RUB по умолчанию для русского, USD, EUR); attributes — массив {key,value,confidence,rawSnippet} с key ТОЛЬКО из реестра: duration (enum day/week/month/permanent: "сутки"→day, "месяц"→month), delete_policy (enum deleted/permanent: "без удаления"→permanent), includes (список строк: "входит ...", "+ доп пост"), tax (строка — НАЛОГ как атрибут, "налог 6%"; НИКОГДА не делай налог отдельным размещением/ценой), notes (строка), tariff_name (строка — «Основной»/«Продвинутый»), slot (строка — «1»/«2»), price_period (enum base/seasonal/promo: обычная→base, «цена июня»/сезон→seasonal, акция→promo), prepayment (строка — «100%»/«50/50»), tax_regime (enum ip/self_employed/ooo/none: «ИП»→ip), tax_included (boolean — «налог включён»→true), top_pin_hours (число — часы в топе), package_items (список строк — состав пакета; пакет = kind=package с ценой пакета); confidence 0..1 (явное с ценой → 0.9+; пакет/неясно → kind=package/other, confidence ≤0.5, но ВСЁ РАВНО верни); rawSnippet — ДОСЛОВНЫЙ фрагмент. "пост на сутки" и "пост на месяц" — ДВА РАЗНЫХ размещения (разный duration), не объединяй.\n\n2) attribute_proposals — если есть условие, которого НЕТ в реестре (duration/delete_policy/includes/tax/notes/tariff_name/slot/price_period/prepayment/tax_regime/tax_included/top_pin_hours/package_items/platform/kind/price/currency), НЕ добавляй как attribute, а предложи: {suggestedKey, suggestedType (string/number/boolean/enum/string_list), applicableKinds, enumValues?, evidence:[verbatim], confidence, rationale}.\n\n3) data_points — ЛЕГАСИ (во время раскатки): "rate.<формат>" латиницей (rate.post/rate.story/rate.reels/rate.video/rate.integration; иначе rate.other), value ЧИСЛО, unit валюта, confidence, rawSnippet verbatim. Верни ПАРАЛЛЕЛЬНО с placement_offers.\n\nНе выдумывай цены/условия. Никогда не выбрасывай неоднозначное молча. JSON: {data_points:[...], placement_offers:[...], attribute_proposals:[...], note?}.',
    userPromptTemplate:
    ...
    // max_tokens raised 1100 → 2200: this extractor now emits THREE
    // representations at once (placement_offers with typed attributes +
    // verbatim rawSnippets, attribute_proposals, and legacy data_points). For
    // multi-format rate cards the old 1100 cap truncated the JSON mid-object,
    // surfacing as `extractJson: unbalanced JSON` (LLM_INVALID_JSON) and
    // failing the whole profile-extract pipeline (conversation → assisted).
  ...
  },
  {
    ...
    model: 'google/gemini-3-flash-preview',
    systemPrompt:
      'Ты извлекаешь СТАТИСТИКУ АУДИТОРИИ блогера: охваты, просмотры, ПОДПИСЧИКОВ ПО ПЛАТФОРМАМ, демографию (пол/возраст), географию. field — одно из: "reach.<формат>" (reach.story/reach.post/reach.reels или просто reach), "views.avg", "audience.subscribers.<платформа>" (ЧИСЛО подписчиков на площадке латиницей: telegram/instagram/vk/tiktok/youtube/max/zen — «Инст 1.2млн»→audience.subscribers.instagram=1200000, «ТГ 15тыс»→audience.subscribers.telegram=15000; это НЕ reach, отдельное поле), "audience.geo", "audience.age", "audience.gender". value — для reach/views/subscribers ЧИСЛО ("12к"→12000, "1.2млн"→1200000); для audience.geo|age|gender — ОБЪЕКТ label→доля/число (например {"Россия":0.7}). unit опционально. confidence — 0..1 (явно → 0.9; неясно — НО ВСЁ РАВНО верни точку). rawSnippet — ДОСЛОВНЫЙ фрагмент (verbatim). Подписчики идут в audience.subscribers.<платформа> (НЕ в reach); если площадка не ясна — пропусти или confidence ≤0.2 + note. Никогда не выбрасывай двусмысленное молча. Возвращай JSON: {data_points: [{field, value, unit?, confidence, rawSnippet}], note?}.',
    userPromptTemplate:
      'Канал: {{channel_title}} (язык: {{language}})\n\nОтветы блогера (свежий — последний):\n{{replies_text}}\n\nСтруктурированный снимок (если есть):\n{{structured_snapshot}}\n{{operator_hints_block}}\nВерни JSON со всеми охватами/просмотрами/подписчиками-по-платформам/демографией/гео как data_points с verbatim rawSnippet.',
  ...
  },
  {
    ...
    role: 'profile-extraction',
    description:
      'OCR изображений-вложений блогера (прайс/медиакит/скриншот статистики) через vision-модель: возвращает распознанный текст дословно (attachment-ocr-ingestion).',
    model: 'google/gemini-3-flash-preview',
    systemPrompt:
      'Ты — OCR. На вход — изображение (прайс блогера, медиакит, скриншот статистики). Распознай и верни ВЕСЬ ТЕКСТ с картинки ДОСЛОВНО, сохраняя строки, цифры, валюты и проценты. НИЧЕГО не придумывай и не добавляй: только то, что реально видно. Не структурируй, не комментируй — просто текст. Если текста нет — верни пустую строку.',
    userPromptTemplate: 'Распознай весь текст с изображения дословно.',
