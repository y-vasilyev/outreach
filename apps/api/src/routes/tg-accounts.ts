...
export async function tgAccountsRoutes(app: FastifyInstance) {
  ...
  app.post('/tg-accounts', async (req) => {
  ...
  app.patch('/tg-accounts/:id', async (req) => {
  ...
  app.delete('/tg-accounts/:id', async (req) => {
    const params = z.object({ id: z.string() }).parse(req.params);
  ...
  app.post('/tg-accounts/:id/login/start', async (req) => {
    const params = z.object({ id: z.string() }).parse(req.params);
    return tgAccountsService.startLogin(params.id);
  });

  app.post('/tg-accounts/:id/login/confirm-code', async (req) => {
    const params = z.object({ id: z.string() }).parse(req.params);
  ...
  app.post('/tg-accounts/:id/login/confirm-password', async (req) => {
    const params = z.object({ id: z.string() }).parse(req.params);
