...
export async function agentsRoutes(app: FastifyInstance) {
  ...
  app.get('/agents', { preHandler: [app.requireRole(['admin'])] }, async () => agentsService.list());

  app.get('/agents/:id', { preHandler: [app.requireRole(['admin'])] }, async (req) => {
  ...
  app.post('/agents/:id/test', { preHandler: [app.requireRole(['admin'])] }, async (req) => {
    const params = z.object({ id: z.string() }).parse(req.params);
    const body = TestAgentInputZ.parse({ id: params.id, ...(req.body as object) });
    return agentsService.test(params.id, body.input);
  });

  app.get('/agents/:id/history', { preHandler: [app.requireRole(['admin'])] }, async (req) => {
    const params = z.object({ id: z.string() }).parse(req.params);
